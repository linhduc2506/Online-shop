...
public class EditBlogController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            user2 = UserDAO.findById(blog.getAuthorid());
        } catch (SQLException ex) {
            Logger.getLogger(EditBlogController.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        request.setAttribute("listCC", listBlogCategory);
        request.setAttribute("blog", blog);
        request.setAttribute("user2", user2);
        request.getRequestDispatcher("EditBlog2.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            BlogDAO.updateBlog(image, title, content, category, id);
        } catch (SQLException ex) {
            Logger.getLogger(EditBlogController.class.getName()).log(Level.SEVERE, null, ex);
        }
        response.sendRedirect(request.getContextPath() + "/editblog?pid=" + id);  
    }
